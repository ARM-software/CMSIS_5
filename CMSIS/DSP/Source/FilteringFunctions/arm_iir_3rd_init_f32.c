 ...
 * Title:        arm_iir_3rd_init_f32.c
 * Description:  3rd order IIR filter initialization function
 *
 * $Date:        
 * $Revision:    
 *
...
void arm_iir_3rd_init_f32(
        ...
        float32_t * pCoeffs )
{
    /* Move [b0 b1 b2 b3 a1 a2 a3] [b0 b1 b2 b3 a1 a2 a3] ... */
    S->pCoeffs = pCoeffs;
    ...
    switch (iirType)
    {
        case ARM_IIR_DF1:
        /* Clear state buffer */
        memset(pState, 0, nbCascaded*6*sizeof(float32_t));
        S->pState = pState;
        ...
        if(simdFlag)
        {
            ...
            pCoeffs += 7*(nbCascaded-1);
            pDstCoeffs += 40*(nbCascaded-1);
            ...
            while (nbCascaded>0)
            {
                ...
                float32_t b3 = pCoeffs[3];
                float32_t a1 = pCoeffs[4];
                float32_t a2 = pCoeffs[5];
                float32_t a3 = pCoeffs[6];
        
                /* Update pointer for previous stage */
                pCoeffs -= 7;
        
                /* Build system */
                float32_t coeffs[4][10] =
                {
                    { 0,  0,  0,  b0, b1, b2, b3, a1, a2, a3 },
                    { 0,  0,  b0, b1, b2, b3, 0,  a2, a3,  0 },
                    { 0,  b0, b1, b2, b3, 0,  0,  a3,  0,  0 },
                ...
                for (int i=0; i<10; i++)
                {
                    ...
                    /* Add a1*row[2] + a2*row[1] + a3*row[0] to row 3 */
                    coeffs[3][i] += a1*coeffs[2][i] + a2*coeffs[1][i] + a3*coeffs[0][i];
                }
                ...
                *pDstCoeffs++ = coeffs[0][8];
                *pDstCoeffs++ = coeffs[1][8];
                *pDstCoeffs++ = coeffs[2][8];
                *pDstCoeffs++ = coeffs[3][8];
        
                *pDstCoeffs++ = coeffs[0][9];
                *pDstCoeffs++ = coeffs[1][9];
                *pDstCoeffs++ = coeffs[2][9];
                *pDstCoeffs++ = coeffs[3][9];

 ...
 * </pre>
 * . 
 * A is a tridiagonal matrix (a band matrix of bandwidth 3) of size N=n+1.<br>
 * The factorization algorithms (A=LU) can be simplified considerably because 
 * a large number of zeros appear in regular patterns. The Crout method has been used:<br>
 * -# Solve LZ=B:
 * <pre>
 * u(1,2) = A(1,2)/A(1,1)
 * z(1)   = B(1)/l(11)
 ...
 * 
 * \par Usage
 * At least 2 known points are needed to perform the cubic spline interpolation,
 * hence n must be bigger or equal than 2.
 * .
 * The x input array must be strictly sorted in ascending order and it must
 * not contain twice the same value (x(i)<x(i+1)).
 * .
 * A temporary buffer of size 3n-1 is needed to store the u(i), z(i) and c(i) values.
 * It has to be allocated by the user and given as input to the initialization function.
 *
 * \par

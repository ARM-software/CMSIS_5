        ...
        uint32_t numSamples)
{
    ...
    while (blkCnt > 0U)
    {
        ...
        newtonStartVec = vdupq_n_s64(INVSQRT_MAGIC_F64) - vshrq((q31x4_t) sum, 1);
        sumHalf = sum * 0.5f;
        /*
         ...
         */
        INVSQRT_NEWTON_MVE_F64(invSqrt, sumHalf, (f64x4_t) newtonStartVec);
        INVSQRT_NEWTON_MVE_F64(invSqrt, sumHalf, invSqrt);
        INVSQRT_NEWTON_MVE_F64(invSqrt, sumHalf, invSqrt);
        /*
    ...
    while (blkCnt > 0U)
    {
      ...
      arm_sqrt_f64((real * real) + (imag * imag), pDst++);
        ...
        uint32_t numSamples)
{
  ...
  while (blkCnt > 0U)
  {
    ...
    vRealA = vmulq_f64(vecA.val[0], vecA.val[0]);
    vImagA = vmulq_f64(vecA.val[1], vecA.val[1]);
    vMagSqA = vaddq_f64(vRealA, vImagA);

    vRealB = vmulq_f64(vecB.val[0], vecB.val[0]);
    vImagB = vmulq_f64(vecB.val[1], vecB.val[1]);
    vMagSqB = vaddq_f64(vRealB, vImagB);

    /* Store the result in the destination buffer. */
    vst1q_f64(pDst, __arm_vec_sqrt_f64_neon(vMagSqA));
    pDst += 4;

    vst1q_f64(pDst, __arm_vec_sqrt_f64_neon(vMagSqB));
    pDst += 4;

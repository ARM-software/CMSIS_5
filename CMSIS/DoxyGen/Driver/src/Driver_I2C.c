...
:---------------------------------------- |:---:|:-----------
\ref ARM_I2C_EVENT_TRANSFER_DONE          |  0  | Occurs after Master/Slave Transmit/Receive operation has finished.
\ref ARM_I2C_EVENT_TRANSFER_INCOMPLETE    |  1  | Occurs together with \ref ARM_I2C_EVENT_TRANSFER_DONE when less data is transferred then requested.
\ref ARM_I2C_EVENT_SLAVE_TRANSMIT         |  2  | Occurs when addressed as Slave Transmitter and \ref ARM_I2C_SlaveTransmit has not been started.
\ref ARM_I2C_EVENT_SLAVE_RECEIVE          |  3  | Occurs when addressed as Slave Receiver and \ref ARM_I2C_SlaveReceive has not been started.
\ref ARM_I2C_EVENT_GENERAL_CALL           |  4  | Indicates General Call in slave mode together with \ref ARM_I2C_EVENT_TRANSFER_DONE, \ref ARM_I2C_EVENT_SLAVE_TRANSMIT and \ref ARM_I2C_EVENT_SLAVE_RECEIVE.
\ref ARM_I2C_EVENT_ADDRESS_NACK           |  5  | Occurs in master mode when address is not acknowledged from slave.
\ref ARM_I2C_EVENT_ARBITRATION_LOST       |  6  | Occurs in master mode when arbitration is lost.
\ref ARM_I2C_EVENT_BUS_ERROR              |  7  | Occurs when bus error is detected.
\ref ARM_I2C_EVENT_BUS_CLEAR              |  8  | Occurs after \ref ARM_I2C_BUS_CLEAR Control operation has finished.

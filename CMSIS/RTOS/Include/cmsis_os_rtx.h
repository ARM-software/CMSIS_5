...
#define osCMSIS           0x20000U     ///< CMSIS-RTOS API version (main [31:16] .sub [15:0])

#define osCMSIS_RTX     ((5<<16)|0)    ///< RTOS identification and version (main [31:16] .sub [15:0])

#define osKernelSystemId "RTX V5.00"   ///< RTOS identification string
...
#define osFeature_Flags        31      ///< 31 Flag bits available
...
typedef struct os_flag_def  {
  void                       *flag;    ///< pointer to internal data
} osFlagDef_t;
...
#else                            // define the object
#define osFlagDef(name)  \
uint32_t os_flag_cb_##name[4] = { 0 }; \
const osFlagDef_t os_flag_def_##name = { (os_flag_cb_##name) }
#endif
...
#if defined(__ARMCC_VERSION) && (__ARMCC_VERSION >= 6010050)
#define   osFlagWait __osFlagWait
osEvent __osFlagWait (osFlagId flag_id, int32_t flags, uint32_t attributes, uint32_t millisec);
#else
os_InRegs osEvent osFlagWait (osFlagId flag_id, int32_t flags, uint32_t attributes, uint32_t millisec);
#endif
...
const osMutexDef_t os_mutex_def_##name = { 1U, (os_mutex_cb_##name) }
#endif
...
#else                            // define the object
#define osMutexDefM(name, instances)  \
uint32_t os_mutex_cb_##name[instances][4] = { 0 }; \
const osMutexDef_t os_mutex_def_##name = { (instances), (os_mutex_cb_##name) }
#endif
...
const osSemaphoreDef_t os_semaphore_def_##name = { 1U, (os_semaphore_cb_##name) }
#endif
...
#else                            // define the object
#define osSemaphoreDefM(name, instances)  \
uint32_t os_semaphore_cb_##name[instances][2] = { 0 }; \
const osSemaphoreDef_t os_semaphore_def_##name = { (instances), (os_semaphore_cb_##name) }
#endif

...
class TestSettings(ABC):
    ...
    def __init__(self, dataset, testtype, args, in_ch, out_ch, x_in, y_in, w_x, w_y, stride_x, stride_y, pad, randmin,
                 randmax, batches=1, generate_bias=True, relu6=False, out_activation_min=None, out_activation_max=None,
        ...
        if self.is_int16xint8:
            ...
            self.output_scale = self.derive_scale_from_min_max(self.out_activation_min, self.out_activation_max,
                                                               INT16_MIN, INT16_MAX)
            ...
            (self.output_scale, self.output_zero_point) = self.derive_scale_and_zeropoint_from_min_max(
                self.out_activation_min, self.out_activation_max)
    ...
    def convert_and_interpret(self, model, input_data, inttype):
        ...
        model_path = "{}model_{}.tflite".format(self.headers_dir, self.testdataset)
        open(model_path, "wb").write(tflite_model)

        interpreter = Interpreter(
            model_path=str(model_path), experimental_op_resolver_type=OpResolverType.BUILTIN_REF)

...
class TestSettings(ABC):
    ...
    # This is input to the data generation. If everything is regenerated then it is overwritten.
    # So it always has the same data as the out dir.
    # The purpouse of the pregen is primarily for debugging, as it enables to change a single parameter and see how the
    # output changes, without regenerating all input data.
    # Another potential use case is to easily switch between testdata sets or loop through all and regenerate them.
...
class FullyConnectedSettings(TestSettings):
    ...
    def __init__(self, args, in_ch=1, out_ch=1, x_in=1, y_in=1, w_x=1, w_y=1, stride_x=1, stride_y=1,
...
if __name__ == '__main__':
    ...
    if args.type == 'conv' or args.type == 'depthwise_conv':
        ...
        #                         pad=True, randmin=-2, randmax=2, outminrange=-126, outmaxrange=127)
    elif args.type == 'fully_connected':
        # fully_connected
        generator = FullyConnectedSettings(args, in_ch=10, out_ch=6, x_in=2, y_in=1, w_x=2, w_y=1,
                                           randmin=-4, randmax=4, batches=3, outminrange=-125.0,
                                           outmaxrange=127.0)
    elif args.type == 'avgpool' or args.type == 'maxpool':
        # avgpooling
        # generator = PoolingSettings(args, channels=8, x_in=22, y_in=12, stride_x=9, stride_y=5, w_x=6, w_y=5,
        #                            pad=True)
        # avgpooling_1
        ...
        # maxpooling_5
        # generator = PoolingSettings(args, channels=20, x_in=1, y_in=1, stride_x=1, stride_y=1, w_x=1, w_y=1,
        #                            pad=True)
        # maxpooling_6

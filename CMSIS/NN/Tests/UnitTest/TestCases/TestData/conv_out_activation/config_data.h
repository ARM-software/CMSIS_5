/*
 * Copyright (C) 2010-2021 Arm Limited or its affiliates. All rights reserved.
 *
 * SPDX-License-Identifier: Apache-2.0
 *
 * Licensed under the Apache License, Version 2.0 (the License); you may
 * not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 *
 * www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an AS IS BASIS, WITHOUT
 * WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */

// Generated by generate_test_data.py using TFL version 2.4.1 as reference.
#pragma once
#define CONV_OUT_ACTIVATION_OUT_CH 2
#define CONV_OUT_ACTIVATION_IN_CH 2
#define CONV_OUT_ACTIVATION_INPUT_W 3
#define CONV_OUT_ACTIVATION_INPUT_H 3
#define CONV_OUT_ACTIVATION_DST_SIZE 18
#define CONV_OUT_ACTIVATION_INPUT_SIZE 18
#define CONV_OUT_ACTIVATION_OUT_ACTIVATION_MIN -55
#define CONV_OUT_ACTIVATION_OUT_ACTIVATION_MAX 55
#define CONV_OUT_ACTIVATION_INPUT_BATCHES 1
#define CONV_OUT_ACTIVATION_INPUT_OFFSET 0
#define CONV_OUT_ACTIVATION_OUTPUT_OFFSET 0
#define CONV_OUT_ACTIVATION_FILTER_X 3
#define CONV_OUT_ACTIVATION_FILTER_Y 3
#define CONV_OUT_ACTIVATION_STRIDE_X 1
#define CONV_OUT_ACTIVATION_STRIDE_Y 1
#define CONV_OUT_ACTIVATION_PAD_X 1
#define CONV_OUT_ACTIVATION_PAD_Y 1
#define CONV_OUT_ACTIVATION_OUTPUT_W 3
#define CONV_OUT_ACTIVATION_OUTPUT_H 3
